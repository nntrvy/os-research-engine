        ...
        switch (name) {
            ...
            case "osr_create_hypothesis": {
                ...
                const { data: project } = await supabase
                    ...
                    .single();
                if (!project)
                    throw new Error(`Project not found: ${params.project_slug}`);
                const { data, error } = await supabase
                    .from("hypotheses")
                    .insert({
            ...
            case "osr_create_experiment": {
                ...
                const { data: hypothesis } = await supabase
                    ...
                    .single();
                if (!hypothesis)
                    throw new Error(`Hypothesis not found: ${params.hypothesis_id}`);
                const { data, error } = await supabase
                    .from("experiments")
                    .insert({
            ...
            case "osr_update_experiment_status": {
                ...
                if (params.success !== undefined)
                    updates.success = params.success;
                if (params.actual_cost)
                    updates.actual_cost = params.actual_cost;
