...
OS Research Engine is the **knowledge base, playbook system, and strategic docs** for OS Research — an independent startup studio focused on building validated startups in Vietnam. This repository contains frameworks, guides, and strategic documents — not application code or project research.

**Note:** All project research, experiment outputs, killed ideas, and validated projects live in `/osr-company/ideas-pipeline/`.

## Repository Structure

```
├── key-research-areas.md          # 5 core problem domains: Education, Insurance, Urban, Health, Culture
├── blue-ocean-opportunities.md    # Living document of potential blue ocean ideas
...
│   ├── osr-strategic-overview.md  # VPCs, BMC, TRR funding model, 10-year target
│   ├── osr-investment-memo.md     # IC investment memo (YC/SV standard)
│   ├── osr-innovation-platform-mlp.md # Platform MLP spec
│   └── osr-manifesto.md           # OS Research manifesto
├── app/                           # Platform prototypes
│   └── landing/                   # Landing page prototype
├── database/                      # Database schemas
└── :init/                         # Context files for AI tools (CLAUDE.md, AGENTS.md, GEMINI.md)
```
...
**CRITICAL:** All research outputs, experiment projects, killed ideas, and validated projects now live in `/osr-company/ideas-pipeline/`.

```bash
ls -la /osr-company/ideas-pipeline/research-outputs/
ls -la /osr-company/ideas-pipeline/experiment-projects/
ls -la /osr-company/ideas-pipeline/killed-ideas/
ls -la /osr-company/ideas-pipeline/validated-projects/
```

Before starting any new research, ALWAYS check if related research already exists in `ideas-pipeline/`.
...
When using the `blue-ocean-researcher` agent or analyzing blue ocean opportunities, reference these 10 classic examples as training data. Full analysis with VPC and BMC is available at `ideas-pipeline/research-outputs/25002-blue-ocean-classics/09.12.25-blue-ocean-classics-analysis.md`.
